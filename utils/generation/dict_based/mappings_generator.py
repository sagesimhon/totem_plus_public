...
class MappingsGenerator:
    ...
    sweep_y: list[float] = field(default_factory=lambda: [-2.0, 2.0])
    sweep_x: list[float] = field(default_factory=lambda: [-2.0, 2.0])
    ...
    def __save_mappings(self, mappings: dict, reverse_mappings: dict, iter_times: list[float], failures: dict, iter_num: int):
        ...
        # failures
        f = os.path.join(self.output_data_path, 'failures.json')
        with open(f, "w") as file:
            json.dump(failures, file)
    ...
    def __load_mappings_if_continuing(self, iter_num: int) -> (dict, dict, list[float]):
        ...
        if iter_num == 0:
            return mappings, reverse_mappings, iter_times, failures
    ...
    def run(self, from_iter=0):
        mappings, reverse_mappings, iter_times, failures = self.__load_mappings_if_continuing(from_iter)
        self.logger.info('Preparing base red dot scene.')
        scene = RedDotScene(resolution_x=self.resolution, resolution_y=self.resolution, spp=self.spp, is_render_only_totem=self.is_render_only_totem, totem=self.totem, red_dot_scale=(0.07, 0.07, 0.07)) #TODO after debug remove red_dot_scale
        scene_sweeper = RedDotSceneSweeper(scene, self.sweep_density, sweep_range_y=self.sweep_y, sweep_range_x=self.sweep_x)
        try:
            if self.is_parallel:
                mappings, reverse_mappings, iter_times, count_failed, _notimplementedfailures = self.run_parallel(scene_sweeper, from_iter)
                raise NotImplementedError("NEW failures dict feature not implemented for python-parallel")
            else:
                mappings, reverse_mappings, iter_times, count_failed, failures = self.run_serial(scene_sweeper, from_iter)
            self.__save_mappings(mappings, reverse_mappings, iter_times, failures, -1)
            if count_failed:
    ...
    def run_serial(self, scene_sweeper: RedDotSceneSweeper, from_iter=0) -> (dict, dict, list[float], int):
        ...
        for i, scene in enumerate(scene_sweeper.scenes(from_iter), start=from_iter):
            ...
            if (i + 1) % self.batch_size_cups_factor == 0:
                ...
                self.__save_mappings(mappings, reverse_mappings, iter_times, failures, i)
        self.__save_mappings(mappings, reverse_mappings, iter_times, failures, -1)
        return mappings, reverse_mappings, iter_times, fail_count, failures
    ...
    def process_red_dot_scene(i: int, base_output_path: str, scene: RedDotScene) -> (dict, dict, float, bool):
        ...
        try:
            ...
            refraction_mapper.render_and_compute_refraction_mappings(is_save_plot=False)
