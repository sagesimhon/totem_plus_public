...
from utils.generation.dict_based.scene import default_world_range_y, default_world_range_x
...
def parse_mappings_arguments():
    ...
    parser.add_argument('--sweep_density', type=float, default=1, help='Granularity of the sweep. Range 0-1 for 0-100% sweep coverage resoution.')
    parser.add_argument('--refresh', type=bool_type, default=True)
    parser.add_argument('--is_render_only_totem', type=bool_type, default=True, help="render only totem (cropped region) for optimal timing. Set to false when debugging and need to see full image")
    parser.add_argument('--p', type=bool_type, default=False, help='True to parallelize python processes. Note that that execution is already parallelized with the rendering backends, but not optimally.')
    ...
    parser.add_argument('--y_max', type=float, default=default_world_range_y[1])
    parser.add_argument('--y_min', type=float, default=default_world_range_y[0])
    parser.add_argument('--x_max', type=float, default=default_world_range_x[1])
    parser.add_argument('--x_min', type=float, default=default_world_range_x[0])
